...
class ScssEditor(models.AbstractModel):
    ...
    def _build_custom_url(self, url_parts, xmlid):
        return f"{url_parts[0]}.custom.{xmlid}.{url_parts[1]}"
    ...
    def get_content(self, url, xmlid):
        ...
        module_path = module.get_module_path(match[1])
        resource_path = f"{match[2]}.{match[5]}"
    ...
    def replace_content(self, url, xmlid, content):
        ...
        if not custom_view.exists():
            ...
            self.env["ir.ui.view"].create(
                {
                    'name': custom_url,
                    'key': f'web_editor.scss_{str(uuid.uuid4())[:6]}',
                    'mode': "extension",
